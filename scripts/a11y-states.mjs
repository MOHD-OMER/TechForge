...
// The topic roadmap flow chart has no progress state to seed — its per-node
// ticks were removed — so it carries no state the standard sweep cannot reach.
// `npm run a11y` covers it; `npm run check:graph` covers its behaviour.
