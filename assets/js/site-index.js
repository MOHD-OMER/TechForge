/**
 * Auto-generated site-wide search index. Regenerate: node tools/build-site-index.mjs
 */
(function () {
  window.TF_SITE_INDEX = [
  {
    "id": "home/404",
    "title": "404 — Page Not Found | TechForge",
    "desc": "404 — Page Not Found | TechForge",
    "icon": "📄",
    "section": "home",
    "sectionKey": "home",
    "href": "404.html",
    "path": "404.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "about/about",
    "title": "About TechForge — Built for Developers, by Developers",
    "desc": "About TechForge — Built for developers, by developers. Interactive CS learning platform covering programming, DSA, syste",
    "icon": "📄",
    "section": "about",
    "sectionKey": "about",
    "href": "about.html",
    "path": "about.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "aiml/aiml-explained",
    "title": "AI / ML",
    "desc": "Complete AI & ML guide — Artificial Intelligence, Machine Learning, Deep Learning, NLP, Computer Vision, Reinforcement L",
    "icon": "🤖",
    "section": "AI/ML",
    "sectionKey": "aiml",
    "href": "aiml/aiml-explained.html",
    "path": "aiml/aiml-explained.html",
    "topic": "aiml/aiml-explained",
    "depth": "guide"
  },
  {
    "id": "aiml/cv",
    "title": "Computer Vision",
    "desc": "Computer Vision",
    "icon": "🤖",
    "section": "AI/ML",
    "sectionKey": "aiml",
    "href": "aiml/cv.html",
    "path": "aiml/cv.html",
    "topic": "aiml/cv",
    "depth": "guide"
  },
  {
    "id": "aiml/dl",
    "title": "Deep Learning",
    "desc": "Deep Learning",
    "icon": "🤖",
    "section": "AI/ML",
    "sectionKey": "aiml",
    "href": "aiml/dl.html",
    "path": "aiml/dl.html",
    "topic": "aiml/dl",
    "depth": "guide"
  },
  {
    "id": "aiml/ds-cheatsheet",
    "title": "DS &amp; AI Cheat Sheet",
    "desc": "DS &amp; AI Cheat Sheet",
    "icon": "🤖",
    "section": "AI/ML",
    "sectionKey": "aiml",
    "href": "aiml/ds-cheatsheet.html",
    "path": "aiml/ds-cheatsheet.html",
    "topic": "aiml/ds-cheatsheet",
    "depth": "guide"
  },
  {
    "id": "aiml/genai",
    "title": "Generative AI",
    "desc": "Generative AI",
    "icon": "🤖",
    "section": "AI/ML",
    "sectionKey": "aiml",
    "href": "aiml/genai.html",
    "path": "aiml/genai.html",
    "topic": "aiml/genai",
    "depth": "guide"
  },
  {
    "id": "aiml/index",
    "title": "AI / ML Hub",
    "desc": "AI & ML learning hub — Machine Learning, Deep Learning, NLP, Computer Vision, Reinforcement Learning, and Generative AI.",
    "icon": "🤖",
    "section": "AI/ML",
    "sectionKey": "aiml",
    "href": "aiml/index.html",
    "path": "aiml/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "aiml/ml",
    "title": "Machine Learning",
    "desc": "Machine Learning",
    "icon": "🤖",
    "section": "AI/ML",
    "sectionKey": "aiml",
    "href": "aiml/ml.html",
    "path": "aiml/ml.html",
    "topic": "aiml/ml",
    "depth": "guide"
  },
  {
    "id": "aiml/nlp",
    "title": "NLP &amp; LLMs",
    "desc": "NLP &amp; LLMs",
    "icon": "🤖",
    "section": "AI/ML",
    "sectionKey": "aiml",
    "href": "aiml/nlp.html",
    "path": "aiml/nlp.html",
    "topic": "aiml/nlp",
    "depth": "guide"
  },
  {
    "id": "aiml/rl",
    "title": "Reinforcement Learning",
    "desc": "Reinforcement Learning",
    "icon": "🤖",
    "section": "AI/ML",
    "sectionKey": "aiml",
    "href": "aiml/rl.html",
    "path": "aiml/rl.html",
    "topic": "aiml/rl",
    "depth": "guide"
  },
  {
    "id": "backend/graphql",
    "title": "GraphQL &amp;amp; gRPC — Backend | TechForge",
    "desc": "GraphQL schemas, resolvers and the N+1 problem, gRPC and protocol buffers, and choosing between REST, GraphQL and gRPC.",
    "icon": "📄",
    "section": "backend",
    "sectionKey": "backend",
    "href": "backend/graphql.html",
    "path": "backend/graphql.html",
    "topic": "backend/graphql",
    "depth": "guide"
  },
  {
    "id": "backend/index",
    "title": "Backend Development",
    "desc": "Building the service behind the API — Node and the event loop, REST and OpenAPI, GraphQL and gRPC, ORMs and migrations, ",
    "icon": "📄",
    "section": "backend",
    "sectionKey": "backend",
    "href": "backend/index.html",
    "path": "backend/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "backend/node",
    "title": "Node &amp;amp; Express — Backend | TechForge",
    "desc": "The Node event loop and non-blocking I/O, Express routing and middleware, error handling, and configuration and process ",
    "icon": "📄",
    "section": "backend",
    "sectionKey": "backend",
    "href": "backend/node.html",
    "path": "backend/node.html",
    "topic": "backend/node",
    "depth": "guide"
  },
  {
    "id": "backend/orm",
    "title": "ORMs &amp;amp; Migrations — Backend | TechForge",
    "desc": "What an ORM does, the N+1 query problem, lazy versus eager loading, connection pooling, and safe schema migrations.",
    "icon": "📄",
    "section": "backend",
    "sectionKey": "backend",
    "href": "backend/orm.html",
    "path": "backend/orm.html",
    "topic": "backend/orm",
    "depth": "guide"
  },
  {
    "id": "backend/rest",
    "title": "REST APIs — Backend | TechForge",
    "desc": "REST resource design, HTTP methods and status codes, idempotency, pagination, versioning strategies, and OpenAPI contrac",
    "icon": "📄",
    "section": "backend",
    "sectionKey": "backend",
    "href": "backend/rest.html",
    "path": "backend/rest.html",
    "topic": "backend/rest",
    "depth": "guide"
  },
  {
    "id": "backend/testing",
    "title": "Backend Testing — Backend | TechForge",
    "desc": "Unit testing backend logic, integration tests with real dependencies and testcontainers, and load testing an API.",
    "icon": "📄",
    "section": "backend",
    "sectionKey": "backend",
    "href": "backend/testing.html",
    "path": "backend/testing.html",
    "topic": "backend/testing",
    "depth": "guide"
  },
  {
    "id": "backend/transactions",
    "title": "Transactions &amp;amp; Isolation — Backend | TechForge",
    "desc": "ACID properties, isolation levels and the anomalies they allow, optimistic versus pessimistic locking, and deadlocks.",
    "icon": "📄",
    "section": "backend",
    "sectionKey": "backend",
    "href": "backend/transactions.html",
    "path": "backend/transactions.html",
    "topic": "backend/transactions",
    "depth": "guide"
  },
  {
    "id": "blockchain/dapps",
    "title": "Building a dApp — Blockchain | TechForge",
    "desc": "Front-end integration with a chain, wallet connection and network handling, and decentralised storage with IPFS and Arwe",
    "icon": "📄",
    "section": "blockchain",
    "sectionKey": "blockchain",
    "href": "blockchain/dapps.html",
    "path": "blockchain/dapps.html",
    "topic": "blockchain/dapps",
    "depth": "guide"
  },
  {
    "id": "blockchain/fundamentals",
    "title": "Blockchain Fundamentals — Blockchain | TechForge",
    "desc": "Blocks and hash chains, Merkle trees, proof of work and proof of stake, wallets and keys, gas and fees, and layer 2 scal",
    "icon": "📄",
    "section": "blockchain",
    "sectionKey": "blockchain",
    "href": "blockchain/fundamentals.html",
    "path": "blockchain/fundamentals.html",
    "topic": "blockchain/fundamentals",
    "depth": "guide"
  },
  {
    "id": "blockchain/index",
    "title": "Blockchain Development",
    "desc": "How a blockchain works, writing smart contracts, the tooling around them, building a dApp, and the security discipline t",
    "icon": "📄",
    "section": "blockchain",
    "sectionKey": "blockchain",
    "href": "blockchain/index.html",
    "path": "blockchain/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "blockchain/security",
    "title": "Smart Contract Security — Blockchain | TechForge",
    "desc": "Reentrancy and the common smart contract bug classes, oracle and economic attacks, and how auditing and formal verificat",
    "icon": "📄",
    "section": "blockchain",
    "sectionKey": "blockchain",
    "href": "blockchain/security.html",
    "path": "blockchain/security.html",
    "topic": "blockchain/security",
    "depth": "guide"
  },
  {
    "id": "blockchain/smart-contracts",
    "title": "Smart Contracts — Blockchain | TechForge",
    "desc": "Solidity, the EVM execution model, ERC token standards, and upgrade patterns for immutable code.",
    "icon": "📄",
    "section": "blockchain",
    "sectionKey": "blockchain",
    "href": "blockchain/smart-contracts.html",
    "path": "blockchain/smart-contracts.html",
    "topic": "blockchain/smart-contracts",
    "depth": "guide"
  },
  {
    "id": "blockchain/tooling",
    "title": "Development Tooling — Blockchain | TechForge",
    "desc": "Hardhat and Foundry, contract testing including fuzzing and forked state, ethers and viem, and node and RPC providers.",
    "icon": "📄",
    "section": "blockchain",
    "sectionKey": "blockchain",
    "href": "blockchain/tooling.html",
    "path": "blockchain/tooling.html",
    "topic": "blockchain/tooling",
    "depth": "guide"
  },
  {
    "id": "programming/basics",
    "title": "C++ Basics — Types, auto &amp; References | TechForge",
    "desc": "C++ Basics — Types, auto &amp; References | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/basics.html",
    "path": "programming/cpp/basics.html",
    "topic": "cpp/basics",
    "depth": "guide"
  },
  {
    "id": "programming/containers",
    "title": "C++ STL Containers — vector, map, set &amp; string | TechForge",
    "desc": "C++ STL Containers — vector, map, set &amp; string | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/containers.html",
    "path": "programming/cpp/containers.html",
    "topic": "cpp/containers",
    "depth": "guide"
  },
  {
    "id": "programming/control",
    "title": "C++ Control Flow — Conditionals, Loops &amp; Range-for | TechForge",
    "desc": "C++ Control Flow — Conditionals, Loops &amp; Range-for | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/control.html",
    "path": "programming/cpp/control.html",
    "topic": "cpp/control",
    "depth": "guide"
  },
  {
    "id": "programming/functions",
    "title": "C++ Functions — Overloading, Default Args &amp; Lambdas | TechForge",
    "desc": "C++ Functions — Overloading, Default Args &amp; Lambdas | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/functions.html",
    "path": "programming/cpp/functions.html",
    "topic": "cpp/functions",
    "depth": "guide"
  },
  {
    "id": "programming/index",
    "title": "C++ Guide",
    "desc": "Complete C++ guide from basics to modern C++ — types, the STL, pointers and memory, OOP with RAII, templates, smart poin",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/index.html",
    "path": "programming/cpp/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "programming/modern",
    "title": "Modern C++ — Smart Pointers, Move Semantics &amp; auto | TechForge",
    "desc": "Modern C++ — Smart Pointers, Move Semantics &amp; auto | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/modern.html",
    "path": "programming/cpp/modern.html",
    "topic": "cpp/modern",
    "depth": "guide"
  },
  {
    "id": "programming/oop",
    "title": "C++ OOP — Classes, RAII, Rule of Five &amp; Virtual | TechForge",
    "desc": "C++ OOP — Classes, RAII, Rule of Five &amp; Virtual | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/oop.html",
    "path": "programming/cpp/oop.html",
    "topic": "cpp/oop",
    "depth": "guide"
  },
  {
    "id": "programming/pointers",
    "title": "C++ Pointers &amp; Memory — Stack, Heap, new/delete | TechForge",
    "desc": "C++ Pointers &amp; Memory — Stack, Heap, new/delete | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/pointers.html",
    "path": "programming/cpp/pointers.html",
    "topic": "cpp/pointers",
    "depth": "guide"
  },
  {
    "id": "programming/programs",
    "title": "C++ Programs — Practice Code Examples | TechForge",
    "desc": "C++ Programs — Practice Code Examples | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/programs.html",
    "path": "programming/cpp/programs.html",
    "topic": "cpp/programs",
    "depth": "guide"
  },
  {
    "id": "programming/templates",
    "title": "C++ Templates &amp; STL Algorithms — Generic Programming | TechForge",
    "desc": "C++ Templates &amp; STL Algorithms — Generic Programming | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "cpp/templates.html",
    "path": "programming/cpp/templates.html",
    "topic": "cpp/templates",
    "depth": "guide"
  },
  {
    "id": "home/dashboard",
    "title": "My Progress",
    "desc": "My Progress",
    "icon": "📄",
    "section": "home",
    "sectionKey": "home",
    "href": "dashboard.html",
    "path": "dashboard.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "data/ab-testing",
    "title": "Experimentation — Data | TechForge",
    "desc": "Experimentation and A/B testing — experiment design, sample size and power, the peeking problem, and reading a result ho",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/ab-testing.html",
    "path": "data/ab-testing.html",
    "topic": "data/ab-testing",
    "depth": "guide"
  },
  {
    "id": "data/bi-tools",
    "title": "BI Tools — Data | TechForge",
    "desc": "Power BI and Tableau compared, semantic layers and metric definitions, and designing dashboards that answer a question.",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/bi-tools.html",
    "path": "data/bi-tools.html",
    "topic": "data/bi-tools",
    "depth": "guide"
  },
  {
    "id": "data/case-studies",
    "title": "Case Study Practice — Data | TechForge",
    "desc": "A framework for data analyst case studies, metric selection, diagnosing a drop, and communicating a recommendation.",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/case-studies.html",
    "path": "data/case-studies.html",
    "topic": "data/case-studies",
    "depth": "guide"
  },
  {
    "id": "data/causal-inference",
    "title": "Causal Inference — Data | TechForge",
    "desc": "Confounding and DAGs, why randomisation works, difference-in-differences, matching, instrumental variables and regressio",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/causal-inference.html",
    "path": "data/causal-inference.html",
    "topic": "data/causal-inference",
    "depth": "guide"
  },
  {
    "id": "data/data-prep",
    "title": "Data Preparation — Data | TechForge",
    "desc": "Data preparation — exploratory analysis, cleaning and missing values, feature engineering, and train/validation/test spl",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/data-prep.html",
    "path": "data/data-prep.html",
    "topic": "data/data-prep",
    "depth": "guide"
  },
  {
    "id": "data/index",
    "title": "Data",
    "desc": "Statistics and experimentation, causal inference, visualisation and BI, data preparation and quality, pipelines and orch",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/index.html",
    "path": "data/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "data/math",
    "title": "Mathematics for ML — Data | TechForge",
    "desc": "Mathematics for machine learning — linear algebra, calculus and gradients, and probability. The working subset, with int",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/math.html",
    "path": "data/math.html",
    "topic": "data/math",
    "depth": "guide"
  },
  {
    "id": "data/mlops",
    "title": "MLOps — Data | TechForge",
    "desc": "MLOps — experiment tracking and reproducibility, model serving patterns, and monitoring for data and concept drift. Inte",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/mlops.html",
    "path": "data/mlops.html",
    "topic": "data/mlops",
    "depth": "guide"
  },
  {
    "id": "data/modelling",
    "title": "Dimensional Modelling — Data | TechForge",
    "desc": "Facts and dimensions, choosing the grain, star versus snowflake, slowly changing dimensions, and denormalisation for ana",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/modelling.html",
    "path": "data/modelling.html",
    "topic": "data/modelling",
    "depth": "guide"
  },
  {
    "id": "data/orchestration",
    "title": "Orchestration &amp;amp; Transformation — Data | TechForge",
    "desc": "Workflow orchestration with Airflow DAGs, scheduling and dependencies, retries and alerting, and dbt for transformation.",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/orchestration.html",
    "path": "data/orchestration.html",
    "topic": "data/orchestration",
    "depth": "guide"
  },
  {
    "id": "data/pipelines",
    "title": "Data Pipelines — Data | TechForge",
    "desc": "Batch versus streaming, ETL and ELT, Spark and distributed processing, idempotency, backfills and late-arriving data.",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/pipelines.html",
    "path": "data/pipelines.html",
    "topic": "data/pipelines",
    "depth": "guide"
  },
  {
    "id": "data/quality",
    "title": "Data Quality Testing — Data | TechForge",
    "desc": "Data quality dimensions, tests in the pipeline, data contracts, freshness and volume monitoring, and anomaly detection.",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/quality.html",
    "path": "data/quality.html",
    "topic": "data/quality",
    "depth": "guide"
  },
  {
    "id": "data/statistics",
    "title": "Statistics — Data | TechForge",
    "desc": "Statistics for data work — descriptive measures, hypothesis testing and p-values, Bayesian thinking, and sampling bias. ",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/statistics.html",
    "path": "data/statistics.html",
    "topic": "data/statistics",
    "depth": "guide"
  },
  {
    "id": "data/visualisation",
    "title": "Visualisation — Data | TechForge",
    "desc": "Data visualisation — choosing a chart, dashboard design, and communicating a result so it gets acted on. Interview quest",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/visualisation.html",
    "path": "data/visualisation.html",
    "topic": "data/visualisation",
    "depth": "guide"
  },
  {
    "id": "data/warehouses",
    "title": "Warehouses &amp; Lakes — Data | TechForge",
    "desc": "Data warehouses and lakes — columnar storage, Parquet and file formats, partitioning, and lakehouse architecture. Interv",
    "icon": "📄",
    "section": "data",
    "sectionKey": "data",
    "href": "data/warehouses.html",
    "path": "data/warehouses.html",
    "topic": "data/warehouses",
    "depth": "guide"
  },
  {
    "id": "databases/cassandra",
    "title": "Cassandra — Database Systems",
    "desc": "Cassandra — Database Systems guide on TechForge. Theory, examples, interview questions, and best practices.",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/cassandra.html",
    "path": "databases/cassandra.html",
    "topic": "db/cassandra",
    "depth": "guide"
  },
  {
    "id": "databases/couchdb",
    "title": "CouchDB — Database Systems",
    "desc": "CouchDB is a document-oriented NoSQL database from Apache. Documents are JSON objects stored with a unique _id and a _re",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/couchdb.html",
    "path": "databases/couchdb.html",
    "topic": "db/couchdb",
    "depth": "guide"
  },
  {
    "id": "databases/dynamodb",
    "title": "DynamoDB — Database Systems",
    "desc": "Amazon DynamoDB is a fully managed, serverless NoSQL database offering single-digit millisecond latency at any scale. Un",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/dynamodb.html",
    "path": "databases/dynamodb.html",
    "topic": "db/dynamodb",
    "depth": "guide"
  },
  {
    "id": "databases/elasticsearch",
    "title": "Elasticsearch (Search) — Database Systems",
    "desc": "Elasticsearch is a distributed search and analytics engine built on Apache Lucene. It stores JSON documents in indices, ",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/elasticsearch.html",
    "path": "databases/elasticsearch.html",
    "topic": "db/elasticsearch",
    "depth": "guide"
  },
  {
    "id": "databases/graph-databases",
    "title": "Graph Databases — Database Systems",
    "desc": "A graph database stores data in a property graph model: nodes represent entities, edges (relationships) represent connec",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/graph-databases.html",
    "path": "databases/graph-databases.html",
    "topic": "db/graph-databases",
    "depth": "guide"
  },
  {
    "id": "databases/index",
    "title": "Database Systems",
    "desc": "Database Systems hub — SQL, PostgreSQL, MongoDB, Redis, graph DBs, and key-value stores. Compare RDBMS vs NoSQL with fre",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/index.html",
    "path": "databases/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "databases/influxdb",
    "title": "InfluxDB (Time Series) — Database Systems",
    "desc": "InfluxDB is a purpose-built time-series database (TSDB) optimized for high-ingest workloads — millions of data points pe",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/influxdb.html",
    "path": "databases/influxdb.html",
    "topic": "db/influxdb",
    "depth": "guide"
  },
  {
    "id": "databases/mariadb",
    "title": "MariaDB — Database Systems",
    "desc": "MariaDB is a community-developed fork of MySQL, created in 2009 when Oracle acquired MySQL. MariaDB maintains drop-in co",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/mariadb.html",
    "path": "databases/mariadb.html",
    "topic": "db/mariadb",
    "depth": "guide"
  },
  {
    "id": "databases/mongodb",
    "title": "MongoDB",
    "desc": "MongoDB cheatsheet — database commands, CRUD operations, query operators, aggregation, and indexes for beginners.",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/mongodb.html",
    "path": "databases/mongodb.html",
    "topic": "db/mongodb",
    "depth": "guide"
  },
  {
    "id": "databases/mysql",
    "title": "MySQL — Database Systems",
    "desc": "MySQL is the world's most deployed open-source relational database. InnoDB is the default storage engine — ACID-complian",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/mysql.html",
    "path": "databases/mysql.html",
    "topic": "db/mysql",
    "depth": "guide"
  },
  {
    "id": "databases/neo4j",
    "title": "Neo4j (Graph) — Database Systems",
    "desc": "Neo4j is a native graph database where data is stored as a property graph: nodes (entities), relationships (directed, ty",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/neo4j.html",
    "path": "databases/neo4j.html",
    "topic": "db/neo4j",
    "depth": "guide"
  },
  {
    "id": "databases/postgres",
    "title": "PostgreSQL — Database Systems",
    "desc": "PostgreSQL is an advanced open-source relational database with full ACID compliance, extensibility, and standards-compli",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/postgres.html",
    "path": "databases/postgres.html",
    "topic": "db/postgres",
    "depth": "guide"
  },
  {
    "id": "databases/redis",
    "title": "Redis — Database Systems",
    "desc": "Redis is an in-memory data structure store used as cache, database, message broker, and queue. Single-threaded event loo",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/redis.html",
    "path": "databases/redis.html",
    "topic": "db/redis",
    "depth": "guide"
  },
  {
    "id": "databases/sql",
    "title": "SQL &amp; Database Guide",
    "desc": "Complete SQL reference — databases, queries, joins, window functions, indexes, normalization and optimization. 40+ inter",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/sql.html",
    "path": "databases/sql.html",
    "topic": "db/sql",
    "depth": "guide"
  },
  {
    "id": "databases/sqlite",
    "title": "SQLite — Database Systems",
    "desc": "SQLite is a serverless, embedded SQL database engine. Unlike PostgreSQL or MySQL, there's no server process — the databa",
    "icon": "🗄️",
    "section": "Database Systems",
    "sectionKey": "databases",
    "href": "databases/sqlite.html",
    "path": "databases/sqlite.html",
    "topic": "db/sqlite",
    "depth": "guide"
  },
  {
    "id": "devops/ansible",
    "title": "Ansible — DevOps",
    "desc": "Ansible connects to managed nodes over SSH (or WinRM) and pushes modules that declare desired state. No agent daemon run",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/ansible.html",
    "path": "devops/ansible.html",
    "topic": "devops/ansible",
    "depth": "guide"
  },
  {
    "id": "devops/aws-basics",
    "title": "AWS Basics — DevOps",
    "desc": "Amazon Web Services spans 30+ regions; each region has multiple Availability Zones (isolated data centers). Place worklo",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/aws-basics.html",
    "path": "devops/aws-basics.html",
    "topic": "devops/aws-basics",
    "depth": "guide"
  },
  {
    "id": "devops/azure-basics",
    "title": "Azure Basics — DevOps",
    "desc": "Azure Basics — DevOps guide on TechForge. Theory, examples, interview questions, and best practices.",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/azure-basics.html",
    "path": "devops/azure-basics.html",
    "topic": "devops/azure-basics",
    "depth": "guide"
  },
  {
    "id": "devops/bash-scripting",
    "title": "Bash Scripting — DevOps",
    "desc": "Bash is the default shell on most Linux systems and macOS. A bash script is a text file starting with #!/bin/bash (the s",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/bash-scripting.html",
    "path": "devops/bash-scripting.html",
    "topic": "devops/bash-scripting",
    "depth": "guide"
  },
  {
    "id": "devops/cicd",
    "title": "CI/CD Overview — DevOps",
    "desc": "CI/CD (Continuous Integration / Continuous Delivery) is the practice of automatically building, testing, and deploying c",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/cicd.html",
    "path": "devops/cicd.html",
    "topic": "devops/cicd",
    "depth": "guide"
  },
  {
    "id": "devops/cloud-fundamentals",
    "title": "Cloud Fundamentals — DevOps",
    "desc": "Cloud computing delivers compute, storage, networking, and services over the internet on a pay-as-you-go model. The thre",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/cloud-fundamentals.html",
    "path": "devops/cloud-fundamentals.html",
    "topic": "devops/cloud-fundamentals",
    "depth": "guide"
  },
  {
    "id": "devops/docker-compose",
    "title": "Docker Compose — DevOps",
    "desc": "Docker Compose defines and runs multi-container applications using a single docker-compose.yml file. Instead of running ",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/docker-compose.html",
    "path": "devops/docker-compose.html",
    "topic": "devops/docker-compose",
    "depth": "guide"
  },
  {
    "id": "devops/docker",
    "title": "Docker — DevOps",
    "desc": "Docker packages applications and dependencies into images — immutable, layered filesystems built from Dockerfile instruc",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/docker.html",
    "path": "devops/docker.html",
    "topic": "devops/docker",
    "depth": "guide"
  },
  {
    "id": "devops/gcp-basics",
    "title": "GCP Basics — DevOps",
    "desc": "Google Cloud Platform (GCP) is Google's cloud. Its strengths are data analytics (BigQuery, Dataflow, Pub/Sub), Kubernete",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/gcp-basics.html",
    "path": "devops/gcp-basics.html",
    "topic": "devops/gcp-basics",
    "depth": "guide"
  },
  {
    "id": "devops/git",
    "title": "Git Commands",
    "desc": "Git Commands for Freshers — The 20 commands that do 95% of the work. Learn branching, merging, rebasing, and the team wo",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/git.html",
    "path": "devops/git.html",
    "topic": "devops/git",
    "depth": "guide"
  },
  {
    "id": "devops/github-actions",
    "title": "GitHub Actions — DevOps",
    "desc": "GitHub Actions is a CI/CD platform built into GitHub. Workflows are YAML files in .github/workflows/ that trigger on rep",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/github-actions.html",
    "path": "devops/github-actions.html",
    "topic": "devops/github-actions",
    "depth": "guide"
  },
  {
    "id": "devops/github",
    "title": "GitHub — DevOps",
    "desc": "GitHub is a cloud-hosted Git repository platform. Beyond storage, it provides pull requests for code review, Issues for ",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/github.html",
    "path": "devops/github.html",
    "topic": "devops/github",
    "depth": "guide"
  },
  {
    "id": "devops/grafana",
    "title": "Grafana — DevOps",
    "desc": "Grafana is an open-source analytics and monitoring platform that visualizes time-series data from multiple sources. It c",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/grafana.html",
    "path": "devops/grafana.html",
    "topic": "devops/grafana",
    "depth": "guide"
  },
  {
    "id": "devops/helm",
    "title": "Helm — DevOps",
    "desc": "Helm is the package manager for Kubernetes. A Helm chart is a collection of Kubernetes YAML templates (Deployment, Servi",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/helm.html",
    "path": "devops/helm.html",
    "topic": "devops/helm",
    "depth": "guide"
  },
  {
    "id": "devops/index",
    "title": "DevOps",
    "desc": "Linux and Git through containers, orchestration and pipelines, out to cloud, observability, reliability practice and sec",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/index.html",
    "path": "devops/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "devops/infrastructure",
    "title": "Infrastructure Basics — DevOps",
    "desc": "Infrastructure as Code (IaC) treats infrastructure configuration (servers, networks, databases, DNS) as version-controll",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/infrastructure.html",
    "path": "devops/infrastructure.html",
    "topic": "devops/infrastructure",
    "depth": "guide"
  },
  {
    "id": "devops/jenkins",
    "title": "Jenkins — DevOps",
    "desc": "Jenkins is an open-source automation server for CI/CD pipelines. It was one of the first CI tools (originally Hudson, 20",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/jenkins.html",
    "path": "devops/jenkins.html",
    "topic": "devops/jenkins",
    "depth": "guide"
  },
  {
    "id": "devops/kubernetes",
    "title": "Kubernetes — DevOps",
    "desc": "Kubernetes orchestrates containers across a cluster of nodes. The control plane (API server, etcd, scheduler, controller",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/kubernetes.html",
    "path": "devops/kubernetes.html",
    "topic": "devops/kubernetes",
    "depth": "guide"
  },
  {
    "id": "devops/linux-basics",
    "title": "Linux Basics — DevOps",
    "desc": "Linux follows the Unix philosophy: everything is a file (devices, sockets, processes in /proc), tools do one thing well,",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/linux-basics.html",
    "path": "devops/linux-basics.html",
    "topic": "devops/linux-basics",
    "depth": "guide"
  },
  {
    "id": "devops/monitoring",
    "title": "Monitoring &amp; Alerts — DevOps",
    "desc": "Monitoring is the practice of collecting, storing, and alerting on metrics that describe system behavior. The four golde",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/monitoring.html",
    "path": "devops/monitoring.html",
    "topic": "devops/monitoring",
    "depth": "guide"
  },
  {
    "id": "devops/nginx",
    "title": "Nginx — DevOps",
    "desc": "Nginx is an event-driven, asynchronous web server and reverse proxy. Unlike Apache's thread-per-request model, Nginx use",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/nginx.html",
    "path": "devops/nginx.html",
    "topic": "devops/nginx",
    "depth": "guide"
  },
  {
    "id": "devops/observability",
    "title": "Observability — DevOps | TechForge",
    "desc": "Observability — structured centralised logging, distributed tracing across services, and alerting that people act on. In",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/observability.html",
    "path": "devops/observability.html",
    "topic": "devops/observability",
    "depth": "guide"
  },
  {
    "id": "devops/prometheus",
    "title": "Prometheus — DevOps",
    "desc": "Prometheus is a pull-based monitoring system: it scrapes HTTP /metrics endpoints on a configured interval (default 15s).",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/prometheus.html",
    "path": "devops/prometheus.html",
    "topic": "devops/prometheus",
    "depth": "guide"
  },
  {
    "id": "devops/reverse-proxy",
    "title": "Reverse Proxy — DevOps",
    "desc": "From a DevOps perspective, a reverse proxy is often the first component in your production stack — it's what receives tr",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/reverse-proxy.html",
    "path": "devops/reverse-proxy.html",
    "topic": "devops/reverse-proxy",
    "depth": "guide"
  },
  {
    "id": "devops/secrets",
    "title": "Secrets &amp; IAM — DevOps | TechForge",
    "desc": "Secrets management and IAM — secret stores, rotation, injection at runtime, and least privilege with roles rather than l",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/secrets.html",
    "path": "devops/secrets.html",
    "topic": "devops/secrets",
    "depth": "guide"
  },
  {
    "id": "devops/slo",
    "title": "SLOs &amp; Reliability — DevOps | TechForge",
    "desc": "SLOs and reliability practice — choosing SLIs, spending an error budget, eliminating toil, humane on-call, blameless pos",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/slo.html",
    "path": "devops/slo.html",
    "topic": "devops/slo",
    "depth": "guide"
  },
  {
    "id": "devops/terraform",
    "title": "Terraform — DevOps",
    "desc": "Terraform is HashiCorp's Infrastructure as Code tool. You write declarative HCL (HashiCorp Configuration Language) descr",
    "icon": "⎇",
    "section": "DevOps",
    "sectionKey": "devops",
    "href": "devops/terraform.html",
    "path": "devops/terraform.html",
    "topic": "devops/terraform",
    "depth": "guide"
  },
  {
    "id": "dsa/arrays",
    "title": "Arrays &amp; Strings",
    "desc": "Arrays &amp; Strings",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/arrays.html",
    "path": "dsa/arrays.html",
    "topic": "dsa/arrays",
    "depth": "guide"
  },
  {
    "id": "dsa/avl",
    "title": "AVL Tree",
    "desc": "AVL Tree",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/avl.html",
    "path": "dsa/avl.html",
    "topic": "dsa/avl",
    "depth": "guide"
  },
  {
    "id": "dsa/bfs",
    "title": "BFS (Breadth-First Search)",
    "desc": "BFS (Breadth-First Search)",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/bfs.html",
    "path": "dsa/bfs.html",
    "topic": "dsa/bfs",
    "depth": "guide"
  },
  {
    "id": "dsa/bigo",
    "title": "Big-O Notation",
    "desc": "Big-O Notation",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/bigo.html",
    "path": "dsa/bigo.html",
    "topic": "dsa/bigo",
    "depth": "guide"
  },
  {
    "id": "dsa/binarysearch",
    "title": "Binary Search",
    "desc": "Binary Search",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/binarysearch.html",
    "path": "dsa/binarysearch.html",
    "topic": "dsa/binarysearch",
    "depth": "guide"
  },
  {
    "id": "dsa/bits",
    "title": "Bit Manipulation",
    "desc": "Bit Manipulation",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/bits.html",
    "path": "dsa/bits.html",
    "topic": "dsa/bits",
    "depth": "guide"
  },
  {
    "id": "dsa/bst",
    "title": "Binary Search Tree",
    "desc": "Binary Search Tree",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/bst.html",
    "path": "dsa/bst.html",
    "topic": "dsa/bst",
    "depth": "guide"
  },
  {
    "id": "dsa/bubble",
    "title": "Bubble Sort",
    "desc": "Bubble Sort",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/bubble.html",
    "path": "dsa/bubble.html",
    "topic": "dsa/bubble",
    "depth": "guide"
  },
  {
    "id": "dsa/cll",
    "title": "Circular Linked List",
    "desc": "Circular Linked List",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/cll.html",
    "path": "dsa/cll.html",
    "topic": "dsa/cll",
    "depth": "guide"
  },
  {
    "id": "dsa/dfs",
    "title": "DFS (Depth-First Search)",
    "desc": "DFS (Depth-First Search)",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/dfs.html",
    "path": "dsa/dfs.html",
    "topic": "dsa/dfs",
    "depth": "guide"
  },
  {
    "id": "dsa/dijkstra",
    "title": "Dijkstra's Algorithm",
    "desc": "Dijkstra's Algorithm",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/dijkstra.html",
    "path": "dsa/dijkstra.html",
    "topic": "dsa/dijkstra",
    "depth": "guide"
  },
  {
    "id": "dsa/dll",
    "title": "Doubly Linked List",
    "desc": "Doubly Linked List",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/dll.html",
    "path": "dsa/dll.html",
    "topic": "dsa/dll",
    "depth": "guide"
  },
  {
    "id": "dsa/dp",
    "title": "Dynamic Programming",
    "desc": "Dynamic Programming",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/dp.html",
    "path": "dsa/dp.html",
    "topic": "dsa/dp",
    "depth": "guide"
  },
  {
    "id": "dsa/graph",
    "title": "Graph",
    "desc": "Graph",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/graph.html",
    "path": "dsa/graph.html",
    "topic": "dsa/graph",
    "depth": "guide"
  },
  {
    "id": "dsa/hashtable",
    "title": "Hash Table",
    "desc": "Hash Table",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/hashtable.html",
    "path": "dsa/hashtable.html",
    "topic": "dsa/hashtable",
    "depth": "guide"
  },
  {
    "id": "dsa/heap",
    "title": "Heap (Priority Queue)",
    "desc": "Heap (Priority Queue)",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/heap.html",
    "path": "dsa/heap.html",
    "topic": "dsa/heap",
    "depth": "guide"
  },
  {
    "id": "dsa/heapsort",
    "title": "Heap Sort",
    "desc": "Heap Sort",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/heapsort.html",
    "path": "dsa/heapsort.html",
    "topic": "dsa/heapsort",
    "depth": "guide"
  },
  {
    "id": "dsa/index",
    "title": "DSA Forge — Interactive Data Structures &amp; Algorithms",
    "desc": "Master Data Structures & Algorithms with live canvas animations, optimized Python code, Big-O analysis, and 100+ curated",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/index.html",
    "path": "dsa/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "dsa/insertion",
    "title": "Insertion Sort",
    "desc": "Insertion Sort",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/insertion.html",
    "path": "dsa/insertion.html",
    "topic": "dsa/insertion",
    "depth": "guide"
  },
  {
    "id": "dsa/linearsearch",
    "title": "Linear Search",
    "desc": "Linear Search",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/linearsearch.html",
    "path": "dsa/linearsearch.html",
    "topic": "dsa/linearsearch",
    "depth": "guide"
  },
  {
    "id": "dsa/logic-building",
    "title": "Logic Building Before DSA — Conditions, Loops, Recursion, Arrays | TechForge",
    "desc": "Logic Building Before DSA — Conditions, Loops, Recursion, Arrays | TechForge",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/logic-building.html",
    "path": "dsa/logic-building.html",
    "topic": "dsa/logic-building",
    "depth": "guide"
  },
  {
    "id": "dsa/merge",
    "title": "Merge Sort",
    "desc": "Merge Sort",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/merge.html",
    "path": "dsa/merge.html",
    "topic": "dsa/merge",
    "depth": "guide"
  },
  {
    "id": "dsa/queue",
    "title": "Queue &amp; Deque",
    "desc": "Queue &amp; Deque",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/queue.html",
    "path": "dsa/queue.html",
    "topic": "dsa/queue",
    "depth": "guide"
  },
  {
    "id": "dsa/quick",
    "title": "Quick Sort",
    "desc": "Quick Sort",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/quick.html",
    "path": "dsa/quick.html",
    "topic": "dsa/quick",
    "depth": "guide"
  },
  {
    "id": "dsa/recursion",
    "title": "Recursion",
    "desc": "Recursion",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/recursion.html",
    "path": "dsa/recursion.html",
    "topic": "dsa/recursion",
    "depth": "guide"
  },
  {
    "id": "dsa/selection",
    "title": "Selection Sort",
    "desc": "Selection Sort",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/selection.html",
    "path": "dsa/selection.html",
    "topic": "dsa/selection",
    "depth": "guide"
  },
  {
    "id": "dsa/sliding",
    "title": "Sliding Window",
    "desc": "Sliding Window",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/sliding.html",
    "path": "dsa/sliding.html",
    "topic": "dsa/sliding",
    "depth": "guide"
  },
  {
    "id": "dsa/sll",
    "title": "Singly Linked List",
    "desc": "Singly Linked List",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/sll.html",
    "path": "dsa/sll.html",
    "topic": "dsa/sll",
    "depth": "guide"
  },
  {
    "id": "dsa/stack",
    "title": "Stack (LIFO)",
    "desc": "Stack (LIFO)",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/stack.html",
    "path": "dsa/stack.html",
    "topic": "dsa/stack",
    "depth": "guide"
  },
  {
    "id": "dsa/trie",
    "title": "Trie (Prefix Tree)",
    "desc": "Trie (Prefix Tree)",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/trie.html",
    "path": "dsa/trie.html",
    "topic": "dsa/trie",
    "depth": "guide"
  },
  {
    "id": "dsa/twopointer",
    "title": "Two Pointer",
    "desc": "Two Pointer",
    "icon": "📊",
    "section": "DSA",
    "sectionKey": "dsa",
    "href": "dsa/twopointer.html",
    "path": "dsa/twopointer.html",
    "topic": "dsa/twopointer",
    "depth": "guide"
  },
  {
    "id": "frontend/accessibility",
    "title": "Accessibility — Frontend | TechForge",
    "desc": "Web accessibility — semantics and ARIA, keyboard and focus management, colour contrast, and screen-reader testing. WCAG ",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/accessibility.html",
    "path": "frontend/accessibility.html",
    "topic": "fe/accessibility",
    "depth": "guide"
  },
  {
    "id": "frontend/css",
    "title": "CSS — Frontend | TechForge",
    "desc": "CSS — the cascade and specificity, box model, Flexbox, Grid, responsive design, custom properties and motion. Theory, ex",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/css.html",
    "path": "frontend/css.html",
    "topic": "fe/css",
    "depth": "guide"
  },
  {
    "id": "frontend/html",
    "title": "HTML — Frontend | TechForge",
    "desc": "HTML — semantic elements, forms and native validation, responsive media, and metadata. Theory, examples, interview quest",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/html.html",
    "path": "frontend/html.html",
    "topic": "fe/html",
    "depth": "guide"
  },
  {
    "id": "frontend/index",
    "title": "Frontend",
    "desc": "The web platform end to end: semantic HTML and the CSS cascade, React, routing and state, server rendering, build toolin",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/index.html",
    "path": "frontend/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "frontend/nextjs",
    "title": "Server Rendering — Frontend | TechForge",
    "desc": "Server rendering for the web — SSR, SSG and incremental regeneration, React server components, and data loading patterns",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/nextjs.html",
    "path": "frontend/nextjs.html",
    "topic": "fe/nextjs",
    "depth": "guide"
  },
  {
    "id": "frontend/performance",
    "title": "Web Performance — Frontend | TechForge",
    "desc": "Web performance — Core Web Vitals (LCP, INP, CLS), bundle size and code splitting, and image and font loading. Interview",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/performance.html",
    "path": "frontend/performance.html",
    "topic": "fe/performance",
    "depth": "guide"
  },
  {
    "id": "frontend/react",
    "title": "React — Frontend | TechForge",
    "desc": "React — components and JSX, props and state, hooks and effects, lists and reconciliation, context and composition, error",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/react.html",
    "path": "frontend/react.html",
    "topic": "fe/react",
    "depth": "guide"
  },
  {
    "id": "frontend/state",
    "title": "Routing &amp; State — Frontend | TechForge",
    "desc": "Frontend routing and state — client-side routing, server state and caching, global state stores, and form state with val",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/state.html",
    "path": "frontend/state.html",
    "topic": "fe/state",
    "depth": "guide"
  },
  {
    "id": "frontend/testing",
    "title": "Testing — Frontend | TechForge",
    "desc": "Frontend testing — unit tests, component tests with Testing Library, and end-to-end tests with Playwright. Test behaviou",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/testing.html",
    "path": "frontend/testing.html",
    "topic": "fe/testing",
    "depth": "guide"
  },
  {
    "id": "frontend/tooling",
    "title": "Build Tooling — Frontend | TechForge",
    "desc": "Frontend build tooling — npm, semver and lockfiles, module bundling with Vite, and ESLint and Prettier. Interview questi",
    "icon": "📄",
    "section": "frontend",
    "sectionKey": "frontend",
    "href": "frontend/tooling.html",
    "path": "frontend/tooling.html",
    "topic": "fe/tooling",
    "depth": "guide"
  },
  {
    "id": "games/engines",
    "title": "Game Engines — Game Development | TechForge",
    "desc": "The game loop, fixed vs variable timestep, entity-component-system architecture, and an honest comparison of Unity, Unre",
    "icon": "📄",
    "section": "games",
    "sectionKey": "games",
    "href": "games/engines.html",
    "path": "games/engines.html",
    "topic": "games/engines",
    "depth": "guide"
  },
  {
    "id": "games/gameplay",
    "title": "Gameplay Systems — Game Development | TechForge",
    "desc": "Input buffering and coyote time, state machines and behaviour trees, pathfinding, game audio, and UI architecture.",
    "icon": "📄",
    "section": "games",
    "sectionKey": "games",
    "href": "games/gameplay.html",
    "path": "games/gameplay.html",
    "topic": "games/gameplay",
    "depth": "guide"
  },
  {
    "id": "games/graphics",
    "title": "Real-Time Graphics — Game Development | TechForge",
    "desc": "The rasterisation pipeline, vertex and fragment shaders, draw calls and batching, and physically based lighting.",
    "icon": "📄",
    "section": "games",
    "sectionKey": "games",
    "href": "games/graphics.html",
    "path": "games/graphics.html",
    "topic": "games/graphics",
    "depth": "guide"
  },
  {
    "id": "games/index",
    "title": "Game Development",
    "desc": "Game maths, engines, real-time graphics, physics, gameplay systems, netcode and shipping — the full path from linear alg",
    "icon": "📄",
    "section": "games",
    "sectionKey": "games",
    "href": "games/index.html",
    "path": "games/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "games/math",
    "title": "Game Maths — Game Development | TechForge",
    "desc": "Vectors, dot and cross products, transform matrices, and why rotations use quaternions instead of three angles.",
    "icon": "📄",
    "section": "games",
    "sectionKey": "games",
    "href": "games/math.html",
    "path": "games/math.html",
    "topic": "games/math",
    "depth": "guide"
  },
  {
    "id": "games/multiplayer",
    "title": "Multiplayer &amp; Netcode — Game Development | TechForge",
    "desc": "Authoritative servers, client prediction and reconciliation, entity interpolation, lag compensation and cheating.",
    "icon": "📄",
    "section": "games",
    "sectionKey": "games",
    "href": "games/multiplayer.html",
    "path": "games/multiplayer.html",
    "topic": "games/multiplayer",
    "depth": "guide"
  },
  {
    "id": "games/physics",
    "title": "Physics &amp; Animation — Game Development | TechForge",
    "desc": "Broad and narrow phase collision, integration and fixed timesteps, tunnelling, and skeletal animation with blending.",
    "icon": "📄",
    "section": "games",
    "sectionKey": "games",
    "href": "games/physics.html",
    "path": "games/physics.html",
    "topic": "games/physics",
    "depth": "guide"
  },
  {
    "id": "games/shipping",
    "title": "Profiling &amp; Shipping — Game Development | TechForge",
    "desc": "Frame budgets, CPU and GPU profiling, asset pipelines and compression, platform certification and release.",
    "icon": "📄",
    "section": "games",
    "sectionKey": "games",
    "href": "games/shipping.html",
    "path": "games/shipping.html",
    "topic": "games/shipping",
    "depth": "guide"
  },
  {
    "id": "interview/aiml-interview",
    "title": "AI/ML Interview Q&amp;A",
    "desc": "AI/ML Interview Q&amp;A",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/aiml-interview.html",
    "path": "interview/aiml-interview.html",
    "topic": "iv/aiml-interview",
    "depth": "guide"
  },
  {
    "id": "interview/cpp-interview",
    "title": "C++ Interview Q&amp;A",
    "desc": "C++ Interview Q&amp;A",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/cpp-interview.html",
    "path": "interview/cpp-interview.html",
    "topic": "iv/cpp-interview",
    "depth": "guide"
  },
  {
    "id": "interview/devops-interview",
    "title": "DevOps Interview Q&amp;A",
    "desc": "29 DevOps interview questions — Docker, Kubernetes, CI/CD, Linux, monitoring, cloud, IaC, and SRE concepts.",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/devops-interview.html",
    "path": "interview/devops-interview.html",
    "topic": "iv/devops-interview",
    "depth": "guide"
  },
  {
    "id": "interview/dsa-interview",
    "title": "DSA Interview Q&amp;A",
    "desc": "47 DSA interview questions — arrays, trees, graphs, DP, sorting — tagged for FAANG and startups. Track your progress loc",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/dsa-interview.html",
    "path": "interview/dsa-interview.html",
    "topic": "iv/dsa-interview",
    "depth": "guide"
  },
  {
    "id": "interview/index",
    "title": "Interview Q&amp;A Hub",
    "desc": "335+ curated interview questions across DSA, Python, JavaScript, Java, C++, OOP, AI/ML, SQL, System Design, DevOps, OS, ",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/index.html",
    "path": "interview/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "interview/java-interview",
    "title": "Java Interview Q&amp;A",
    "desc": "Java Interview Q&amp;A",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/java-interview.html",
    "path": "interview/java-interview.html",
    "topic": "iv/java-interview",
    "depth": "guide"
  },
  {
    "id": "interview/js-interview",
    "title": "JavaScript Interview Q&amp;A",
    "desc": "JavaScript Interview Q&amp;A",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/js-interview.html",
    "path": "interview/js-interview.html",
    "topic": "iv/js-interview",
    "depth": "guide"
  },
  {
    "id": "interview/networking-interview",
    "title": "Networking Interview Q&amp;A",
    "desc": "30 networking interview questions — TCP/IP, TCP vs UDP, DNS, HTTP/HTTPS, sockets, subnets, NAT and real-time protocols.",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/networking-interview.html",
    "path": "interview/networking-interview.html",
    "topic": "iv/networking-interview",
    "depth": "guide"
  },
  {
    "id": "interview/oop-interview",
    "title": "OOP Interview Q&amp;A",
    "desc": "OOP Interview Q&amp;A",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/oop-interview.html",
    "path": "interview/oop-interview.html",
    "topic": "iv/oop-interview",
    "depth": "guide"
  },
  {
    "id": "interview/os-interview",
    "title": "OS Interview Q&amp;A",
    "desc": "30 operating systems interview questions — processes, threads, scheduling, memory, concurrency, deadlocks, filesystems a",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/os-interview.html",
    "path": "interview/os-interview.html",
    "topic": "iv/os-interview",
    "depth": "guide"
  },
  {
    "id": "interview/python-interview",
    "title": "Python Interview Q&amp;A",
    "desc": "Python Interview Q&amp;A",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/python-interview.html",
    "path": "interview/python-interview.html",
    "topic": "iv/python-interview",
    "depth": "guide"
  },
  {
    "id": "interview/security-interview",
    "title": "Security Interview Q&amp;A",
    "desc": "30 security interview questions — CIA triad, cryptography, authentication, OWASP web attacks, TLS, network security and ",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/security-interview.html",
    "path": "interview/security-interview.html",
    "topic": "iv/security-interview",
    "depth": "guide"
  },
  {
    "id": "interview/sql-interview",
    "title": "SQL Interview Q&amp;A",
    "desc": "SQL Interview Q&amp;A",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/sql-interview.html",
    "path": "interview/sql-interview.html",
    "topic": "iv/sql-interview",
    "depth": "guide"
  },
  {
    "id": "interview/system-design-interview",
    "title": "System Design Interview Q&amp;A",
    "desc": "38 system design interview questions — scalability, caching, CAP, microservices, case studies — tagged Beginner to FAANG",
    "icon": "🎯",
    "section": "Interview",
    "sectionKey": "interview",
    "href": "interview/system-design-interview.html",
    "path": "interview/system-design-interview.html",
    "topic": "iv/system-design-interview",
    "depth": "guide"
  },
  {
    "id": "programming/basics",
    "title": "Java Basics — Primitives, var &amp; Strings | TechForge",
    "desc": "Java Basics — Primitives, var &amp; Strings | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/basics.html",
    "path": "programming/java/basics.html",
    "topic": "java/basics",
    "depth": "guide"
  },
  {
    "id": "programming/collections",
    "title": "Java Collections — List, Map, Set &amp; Generics | TechForge",
    "desc": "Java Collections — List, Map, Set &amp; Generics | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/collections.html",
    "path": "programming/java/collections.html",
    "topic": "java/collections",
    "depth": "guide"
  },
  {
    "id": "programming/concurrency",
    "title": "Java Concurrency — Threads, Executors &amp; synchronized | TechForge",
    "desc": "Java Concurrency — Threads, Executors &amp; synchronized | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/concurrency.html",
    "path": "programming/java/concurrency.html",
    "topic": "java/concurrency",
    "depth": "guide"
  },
  {
    "id": "programming/control",
    "title": "Java Control Flow — Conditionals, Loops &amp; switch | TechForge",
    "desc": "Java Control Flow — Conditionals, Loops &amp; switch | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/control.html",
    "path": "programming/java/control.html",
    "topic": "java/control",
    "depth": "guide"
  },
  {
    "id": "programming/exceptions",
    "title": "Java Exceptions &amp; File I/O — try/catch, Checked vs Unchecked | TechForge",
    "desc": "Java Exceptions &amp; File I/O — try/catch, Checked vs Unchecked | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/exceptions.html",
    "path": "programming/java/exceptions.html",
    "topic": "java/exceptions",
    "depth": "guide"
  },
  {
    "id": "programming/index",
    "title": "Java Guide",
    "desc": "Complete Java guide from basics to concurrency — types, collections, OOP with all four pillars, generics, streams and la",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/index.html",
    "path": "programming/java/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "programming/methods",
    "title": "Java Methods &amp; Classes — Overloading, static &amp; Constructors | TechForge",
    "desc": "Java Methods &amp; Classes — Overloading, static &amp; Constructors | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/methods.html",
    "path": "programming/java/methods.html",
    "topic": "java/methods",
    "depth": "guide"
  },
  {
    "id": "programming/oop",
    "title": "Java OOP — Inheritance, Interfaces &amp; Records | TechForge",
    "desc": "Java OOP — Inheritance, Interfaces &amp; Records | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/oop.html",
    "path": "programming/java/oop.html",
    "topic": "java/oop",
    "depth": "guide"
  },
  {
    "id": "programming/programs",
    "title": "Java Programs — Practice Code Examples | TechForge",
    "desc": "Java Programs — Practice Code Examples | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/programs.html",
    "path": "programming/java/programs.html",
    "topic": "java/programs",
    "depth": "guide"
  },
  {
    "id": "programming/streams",
    "title": "Java Streams &amp; Lambdas — Functional Java | TechForge",
    "desc": "Java Streams &amp; Lambdas — Functional Java | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "java/streams.html",
    "path": "programming/java/streams.html",
    "topic": "java/streams",
    "depth": "guide"
  },
  {
    "id": "programming/async",
    "title": "JavaScript Async / Await — Promises, Event Loop &amp; Concurrency | TechForge",
    "desc": "JavaScript Async / Await — Promises, Event Loop &amp; Concurrency | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/async.html",
    "path": "programming/javascript/async.html",
    "topic": "js/async",
    "depth": "guide"
  },
  {
    "id": "programming/basics",
    "title": "JavaScript Basics — Variables &amp; Data Types | TechForge",
    "desc": "JavaScript Basics — Variables &amp; Data Types | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/basics.html",
    "path": "programming/javascript/basics.html",
    "topic": "js/basics",
    "depth": "guide"
  },
  {
    "id": "programming/collections",
    "title": "JavaScript Collections — Arrays, Objects, Map &amp; Set | TechForge",
    "desc": "JavaScript Collections — Arrays, Objects, Map &amp; Set | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/collections.html",
    "path": "programming/javascript/collections.html",
    "topic": "js/collections",
    "depth": "guide"
  },
  {
    "id": "programming/control",
    "title": "JavaScript Control Flow — Conditionals &amp; Loops | TechForge",
    "desc": "JavaScript Control Flow — Conditionals &amp; Loops | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/control.html",
    "path": "programming/javascript/control.html",
    "topic": "js/control",
    "depth": "guide"
  },
  {
    "id": "programming/dom",
    "title": "JavaScript DOM &amp; Browser — Elements, Events, Fetch | TechForge",
    "desc": "JavaScript DOM &amp; Browser — Elements, Events, Fetch | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/dom.html",
    "path": "programming/javascript/dom.html",
    "topic": "js/dom",
    "depth": "guide"
  },
  {
    "id": "programming/functions",
    "title": "JavaScript Functions, Closures &amp; this | TechForge",
    "desc": "JavaScript Functions, Closures &amp; this | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/functions.html",
    "path": "programming/javascript/functions.html",
    "topic": "js/functions",
    "depth": "guide"
  },
  {
    "id": "programming/index",
    "title": "JavaScript Guide",
    "desc": "Complete JavaScript guide from basics to async — closures, prototypes, classes, promises, the DOM, and the npm ecosystem",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/index.html",
    "path": "programming/javascript/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "programming/modules",
    "title": "JavaScript Modules &amp; Ecosystem — npm, Bundlers, Libraries | TechForge",
    "desc": "JavaScript Modules &amp; Ecosystem — npm, Bundlers, Libraries | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/modules.html",
    "path": "programming/javascript/modules.html",
    "topic": "js/modules",
    "depth": "guide"
  },
  {
    "id": "programming/oop",
    "title": "JavaScript OOP — Prototypes, Classes &amp; Inheritance | TechForge",
    "desc": "JavaScript OOP — Prototypes, Classes &amp; Inheritance | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/oop.html",
    "path": "programming/javascript/oop.html",
    "topic": "js/oop",
    "depth": "guide"
  },
  {
    "id": "programming/programs",
    "title": "JavaScript Programs — Practice Code Examples | TechForge",
    "desc": "JavaScript Programs — Practice Code Examples | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "javascript/programs.html",
    "path": "programming/javascript/programs.html",
    "topic": "js/programs",
    "depth": "guide"
  },
  {
    "id": "mobile/architecture",
    "title": "State &amp;amp; Architecture — Mobile Development | TechForge",
    "desc": "MVVM and MVI, unidirectional data flow, dependency injection, and background work on mobile.",
    "icon": "📄",
    "section": "mobile",
    "sectionKey": "mobile",
    "href": "mobile/architecture.html",
    "path": "mobile/architecture.html",
    "topic": "mobile/architecture",
    "depth": "guide"
  },
  {
    "id": "mobile/cross-platform",
    "title": "Cross-Platform — Mobile Development | TechForge",
    "desc": "React Native and Flutter compared: rendering models, shared code, native modules, and when cross-platform is the wrong c",
    "icon": "📄",
    "section": "mobile",
    "sectionKey": "mobile",
    "href": "mobile/cross-platform.html",
    "path": "mobile/cross-platform.html",
    "topic": "mobile/cross-platform",
    "depth": "guide"
  },
  {
    "id": "mobile/index",
    "title": "Mobile Development",
    "desc": "Native and cross-platform mobile — platforms and lifecycle, declarative UI, architecture, offline networking, local stor",
    "icon": "📄",
    "section": "mobile",
    "sectionKey": "mobile",
    "href": "mobile/index.html",
    "path": "mobile/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "mobile/networking",
    "title": "Networking &amp;amp; Offline — Mobile Development | TechForge",
    "desc": "REST clients on mobile, retries and timeouts, caching, offline-first design and sync conflict resolution.",
    "icon": "📄",
    "section": "mobile",
    "sectionKey": "mobile",
    "href": "mobile/networking.html",
    "path": "mobile/networking.html",
    "topic": "mobile/networking",
    "depth": "guide"
  },
  {
    "id": "mobile/platforms",
    "title": "Choosing a Platform — Mobile Development | TechForge",
    "desc": "Native versus cross-platform, Kotlin and Android, Swift and iOS, and the app lifecycle that has no desktop equivalent.",
    "icon": "📄",
    "section": "mobile",
    "sectionKey": "mobile",
    "href": "mobile/platforms.html",
    "path": "mobile/platforms.html",
    "topic": "mobile/platforms",
    "depth": "guide"
  },
  {
    "id": "mobile/release",
    "title": "Testing &amp;amp; Release — Mobile Development | TechForge",
    "desc": "Unit and UI testing on mobile, app store submission and review, staged rollouts, crash reporting and analytics.",
    "icon": "📄",
    "section": "mobile",
    "sectionKey": "mobile",
    "href": "mobile/release.html",
    "path": "mobile/release.html",
    "topic": "mobile/release",
    "depth": "guide"
  },
  {
    "id": "mobile/storage",
    "title": "Local Storage — Mobile Development | TechForge",
    "desc": "Key-value preferences, on-device databases with Room and Core Data, files, migrations, and secure storage in Keychain an",
    "icon": "📄",
    "section": "mobile",
    "sectionKey": "mobile",
    "href": "mobile/storage.html",
    "path": "mobile/storage.html",
    "topic": "mobile/storage",
    "depth": "guide"
  },
  {
    "id": "mobile/ui",
    "title": "Building the UI — Mobile Development | TechForge",
    "desc": "Jetpack Compose and SwiftUI, navigation, responsive layout across screen sizes, and mobile accessibility.",
    "icon": "📄",
    "section": "mobile",
    "sectionKey": "mobile",
    "href": "mobile/ui.html",
    "path": "mobile/ui.html",
    "topic": "mobile/ui",
    "depth": "guide"
  },
  {
    "id": "systems/dns",
    "title": "DNS — Networking | TechForge",
    "desc": "DNS — Networking | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "networking/dns.html",
    "path": "systems/networking/dns.html",
    "topic": "sys/net-dns",
    "depth": "guide"
  },
  {
    "id": "systems/http",
    "title": "HTTP &amp; HTTPS — Networking | TechForge",
    "desc": "HTTP &amp; HTTPS — Networking | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "networking/http.html",
    "path": "systems/networking/http.html",
    "topic": "sys/net-http",
    "depth": "guide"
  },
  {
    "id": "systems/index",
    "title": "Networking",
    "desc": "Networking roadmap — the TCP/IP stack, TCP vs UDP, DNS, HTTP &amp; HTTPS, sockets, subnets and WebSockets. Eight full gu",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "networking/index.html",
    "path": "systems/networking/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "systems/realtime",
    "title": "WebSockets &amp; Real-Time — Networking | TechForge",
    "desc": "WebSockets &amp; Real-Time — Networking | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "networking/realtime.html",
    "path": "systems/networking/realtime.html",
    "topic": "sys/net-realtime",
    "depth": "guide"
  },
  {
    "id": "systems/routing",
    "title": "IP, Subnets &amp; Routing — Networking | TechForge",
    "desc": "IP, Subnets &amp; Routing — Networking | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "networking/routing.html",
    "path": "systems/networking/routing.html",
    "topic": "sys/net-routing",
    "depth": "guide"
  },
  {
    "id": "systems/sockets",
    "title": "Sockets &amp; Ports — Networking | TechForge",
    "desc": "Sockets &amp; Ports — Networking | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "networking/sockets.html",
    "path": "systems/networking/sockets.html",
    "topic": "sys/net-sockets",
    "depth": "guide"
  },
  {
    "id": "systems/stack",
    "title": "The TCP/IP Stack — Networking | TechForge",
    "desc": "The TCP/IP Stack — Networking | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "networking/stack.html",
    "path": "systems/networking/stack.html",
    "topic": "sys/net-stack",
    "depth": "guide"
  },
  {
    "id": "systems/tcp-udp",
    "title": "TCP vs UDP — Networking | TechForge",
    "desc": "TCP vs UDP — Networking | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "networking/tcp-udp.html",
    "path": "systems/networking/tcp-udp.html",
    "topic": "sys/net-tcp-udp",
    "depth": "guide"
  },
  {
    "id": "systems/url-journey",
    "title": "What Happens When You Type a URL — Networking | TechForge",
    "desc": "What Happens When You Type a URL — Networking | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "networking/url-journey.html",
    "path": "systems/networking/url-journey.html",
    "topic": "sys/net-url-journey",
    "depth": "guide"
  },
  {
    "id": "home/offline",
    "title": "Offline",
    "desc": "Offline",
    "icon": "📄",
    "section": "home",
    "sectionKey": "home",
    "href": "offline.html",
    "path": "offline.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "systems/boot",
    "title": "Boot to Shell — Operating Systems | TechForge",
    "desc": "Boot to Shell — Operating Systems | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "os/boot.html",
    "path": "systems/os/boot.html",
    "topic": "sys/os-boot",
    "depth": "guide"
  },
  {
    "id": "systems/concurrency",
    "title": "Concurrency — Operating Systems | TechForge",
    "desc": "Concurrency — Operating Systems | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "os/concurrency.html",
    "path": "systems/os/concurrency.html",
    "topic": "sys/os-concurrency",
    "depth": "guide"
  },
  {
    "id": "systems/deadlocks",
    "title": "Deadlocks — Operating Systems | TechForge",
    "desc": "Deadlocks — Operating Systems | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "os/deadlocks.html",
    "path": "systems/os/deadlocks.html",
    "topic": "sys/os-deadlocks",
    "depth": "guide"
  },
  {
    "id": "systems/filesystems",
    "title": "Filesystems — Operating Systems | TechForge",
    "desc": "Filesystems — Operating Systems | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "os/filesystems.html",
    "path": "systems/os/filesystems.html",
    "topic": "sys/os-filesystems",
    "depth": "guide"
  },
  {
    "id": "systems/index",
    "title": "Operating Systems",
    "desc": "Operating Systems roadmap — processes and threads, CPU scheduling, virtual memory, filesystems, concurrency and deadlock",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "os/index.html",
    "path": "systems/os/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "systems/io",
    "title": "I/O &amp; Interrupts — Operating Systems | TechForge",
    "desc": "I/O &amp; Interrupts — Operating Systems | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "os/io.html",
    "path": "systems/os/io.html",
    "topic": "sys/os-io",
    "depth": "guide"
  },
  {
    "id": "systems/memory",
    "title": "Memory Management — Operating Systems | TechForge",
    "desc": "Memory Management — Operating Systems | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "os/memory.html",
    "path": "systems/os/memory.html",
    "topic": "sys/os-memory",
    "depth": "guide"
  },
  {
    "id": "systems/processes",
    "title": "Processes &amp; Threads — Operating Systems | TechForge",
    "desc": "Processes &amp; Threads — Operating Systems | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "os/processes.html",
    "path": "systems/os/processes.html",
    "topic": "sys/os-processes",
    "depth": "guide"
  },
  {
    "id": "systems/scheduling",
    "title": "CPU Scheduling — Operating Systems | TechForge",
    "desc": "CPU Scheduling — Operating Systems | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "os/scheduling.html",
    "path": "systems/os/scheduling.html",
    "topic": "sys/os-scheduling",
    "depth": "guide"
  },
  {
    "id": "roadmaps/aiml",
    "title": "AI / ML Engineer Roadmap",
    "desc": "Maths, Python and the data stack first, then classical ML, deep learning, the specialisations, and the applied and MLOps",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/aiml.html",
    "path": "roadmaps/paths/aiml.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/backend",
    "title": "Backend Developer Roadmap",
    "desc": "The full server-side path: a language, the Linux and network layer under it, SQL and NoSQL, APIs and auth, then scale, q",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/backend.html",
    "path": "roadmaps/paths/backend.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/blockchain-developer",
    "title": "Blockchain Developer Roadmap",
    "desc": "Cryptography and distributed consensus first, then smart contracts, the tooling around them, and the security practice a",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/blockchain-developer.html",
    "path": "roadmaps/paths/blockchain-developer.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/cloud-engineer",
    "title": "Cloud Engineer Roadmap",
    "desc": "Linux and networking underneath, then one provider learned properly, infrastructure as code, containers, and the securit",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/cloud-engineer.html",
    "path": "roadmaps/paths/cloud-engineer.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/cs",
    "title": "CS Fundamentals Roadmap",
    "desc": "A computer science curriculum in order: programming, data structures and algorithms, operating systems, networks, databa",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/cs.html",
    "path": "roadmaps/paths/cs.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/data-analyst",
    "title": "Data Analyst Roadmap",
    "desc": "Spreadsheets and SQL first, then Python and the statistics to make a number mean something, and the visualisation and re",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/data-analyst.html",
    "path": "roadmaps/paths/data-analyst.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/data-engineer",
    "title": "Data Engineer Roadmap",
    "desc": "The pipelines everyone else depends on: SQL and Python, warehouses and modelling, batch and streaming, and the orchestra",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/data-engineer.html",
    "path": "roadmaps/paths/data-engineer.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/data-scientist",
    "title": "Data Scientist Roadmap",
    "desc": "Statistics and Python underneath, modelling and experimentation on top, and the communication that turns a model into a ",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/data-scientist.html",
    "path": "roadmaps/paths/data-scientist.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/devops",
    "title": "DevOps &amp; Cloud Roadmap",
    "desc": "From a Linux box to a production platform: containers, Kubernetes, infrastructure as code, cloud, pipelines, observabili",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/devops.html",
    "path": "roadmaps/paths/devops.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/dsa",
    "title": "DSA &amp; Interview Prep Roadmap",
    "desc": "The order to learn data structures and algorithms in, from logic building and Big-O through to dynamic programming, then",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/dsa.html",
    "path": "roadmaps/paths/dsa.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/frontend",
    "title": "Frontend Developer Roadmap",
    "desc": "The full frontend path: HTML and CSS, JavaScript, how the browser fetches a page, Git, build tooling, TypeScript, React,",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/frontend.html",
    "path": "roadmaps/paths/frontend.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/fullstack",
    "title": "Full Stack Developer Roadmap",
    "desc": "HTML, CSS and JavaScript, then TypeScript and React on the client, an API and a database on the server, and the auth, te",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/fullstack.html",
    "path": "roadmaps/paths/fullstack.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/game-developer",
    "title": "Game Developer Roadmap",
    "desc": "A systems language and the maths behind everything on screen, then an engine, graphics and physics, and the profiling an",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/game-developer.html",
    "path": "roadmaps/paths/game-developer.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/mobile-developer",
    "title": "Mobile Developer Roadmap",
    "desc": "One platform learned properly — Android or iOS — then the UI, storage, networking and release process that every mobile ",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/mobile-developer.html",
    "path": "roadmaps/paths/mobile-developer.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/qa-engineer",
    "title": "QA Engineer Roadmap",
    "desc": "Test design and manual technique first, then automation at every level, API and performance testing, and the pipeline th",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/qa-engineer.html",
    "path": "roadmaps/paths/qa-engineer.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/security-engineer",
    "title": "Security Engineer Roadmap",
    "desc": "Networks and operating systems first — you cannot defend what you do not understand — then cryptography, application and",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/security-engineer.html",
    "path": "roadmaps/paths/security-engineer.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/software-architect",
    "title": "Software Architect Roadmap",
    "desc": "Years of building first, then the distributed-systems knowledge, trade-off vocabulary and communication skills to make d",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/software-architect.html",
    "path": "roadmaps/paths/software-architect.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/sre",
    "title": "Site Reliability Engineer Roadmap",
    "desc": "Software engineering applied to operations: the systems underneath, the observability to see them, and the SLOs, automat",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "paths/sre.html",
    "path": "roadmaps/paths/sre.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "programming/index",
    "title": "Programming Languages",
    "desc": "Programming Languages hub — learn how languages turn ideas into instructions. Five full tracks are live: Python, JavaScr",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "programming/index.html",
    "path": "programming/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "programming/async",
    "title": "Python Async / Await — asyncio, Coroutines &amp; Concurrency | TechForge",
    "desc": "Python Async / Await — asyncio, Coroutines &amp; Concurrency | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/async.html",
    "path": "programming/python/async.html",
    "topic": "py/async",
    "depth": "guide"
  },
  {
    "id": "programming/basics",
    "title": "Python Basics — Variables &amp; Data Types | TechForge",
    "desc": "Python Basics — Variables &amp; Data Types | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/basics.html",
    "path": "programming/python/basics.html",
    "topic": "py/basics",
    "depth": "guide"
  },
  {
    "id": "programming/collections",
    "title": "Python Collections — Lists, Tuples, Sets, Dictionaries | TechForge",
    "desc": "Python Collections — Lists, Tuples, Sets, Dictionaries | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/collections.html",
    "path": "programming/python/collections.html",
    "topic": "py/collections",
    "depth": "guide"
  },
  {
    "id": "programming/control",
    "title": "Python Control Flow — Conditionals &amp; Loops | TechForge",
    "desc": "Python Control Flow — Conditionals &amp; Loops | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/control.html",
    "path": "programming/python/control.html",
    "topic": "py/control",
    "depth": "guide"
  },
  {
    "id": "programming/functions",
    "title": "Python Functions — Recursion, Exceptions, File Handling | TechForge",
    "desc": "Python Functions — Recursion, Exceptions, File Handling | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/functions.html",
    "path": "programming/python/functions.html",
    "topic": "py/functions",
    "depth": "guide"
  },
  {
    "id": "programming/index",
    "title": "Python Guide",
    "desc": "Complete Python guide from basics to advanced OOP — decorators, generators, comprehensions, magic methods, and essential",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/index.html",
    "path": "programming/python/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "programming/libraries",
    "title": "Python Libraries — NumPy, Pandas, Matplotlib &amp; More | TechForge",
    "desc": "Python Libraries — NumPy, Pandas, Matplotlib &amp; More | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/libraries.html",
    "path": "programming/python/libraries.html",
    "topic": "py/libraries",
    "depth": "guide"
  },
  {
    "id": "programming/modules",
    "title": "Python Modules &amp; Packaging — Imports, Packages, Project Layout | TechForge",
    "desc": "Python Modules &amp; Packaging — Imports, Packages, Project Layout | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/modules.html",
    "path": "programming/python/modules.html",
    "topic": "py/modules",
    "depth": "guide"
  },
  {
    "id": "programming/oop",
    "title": "Python OOP — Complete Object-Oriented Programming | TechForge",
    "desc": "Python OOP — Complete Object-Oriented Programming | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/oop.html",
    "path": "programming/python/oop.html",
    "topic": "py/oop",
    "depth": "guide"
  },
  {
    "id": "programming/programs",
    "title": "Python Programs — Practice Code Examples | TechForge",
    "desc": "Python Programs — Practice Code Examples | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "python/programs.html",
    "path": "programming/python/programs.html",
    "topic": "py/programs",
    "depth": "guide"
  },
  {
    "id": "qa/api-testing",
    "title": "API Testing — QA &amp; Testing | TechForge",
    "desc": "Testing REST APIs, status codes and schema validation, contract testing between services, and Postman and REST clients.",
    "icon": "📄",
    "section": "qa",
    "sectionKey": "qa",
    "href": "qa/api-testing.html",
    "path": "qa/api-testing.html",
    "topic": "qa/api-testing",
    "depth": "guide"
  },
  {
    "id": "qa/automation",
    "title": "Unit &amp;amp; Integration Tests — QA &amp; Testing | TechForge",
    "desc": "Test frameworks and structure, mocking and test doubles, fixtures and test data, and what makes a suite trustworthy.",
    "icon": "📄",
    "section": "qa",
    "sectionKey": "qa",
    "href": "qa/automation.html",
    "path": "qa/automation.html",
    "topic": "qa/automation",
    "depth": "guide"
  },
  {
    "id": "qa/fundamentals",
    "title": "Testing Fundamentals — QA &amp; Testing | TechForge",
    "desc": "Types of testing, the testing pyramid, writing test cases, and bug reports that get acted on.",
    "icon": "📄",
    "section": "qa",
    "sectionKey": "qa",
    "href": "qa/fundamentals.html",
    "path": "qa/fundamentals.html",
    "topic": "qa/fundamentals",
    "depth": "guide"
  },
  {
    "id": "qa/index",
    "title": "QA &amp; Testing",
    "desc": "Testing fundamentals and the pyramid, exploratory and regression testing, unit and integration automation, UI and API te",
    "icon": "📄",
    "section": "qa",
    "sectionKey": "qa",
    "href": "qa/index.html",
    "path": "qa/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "qa/manual",
    "title": "Manual Testing — QA &amp; Testing | TechForge",
    "desc": "Exploratory testing with charters and session notes, regression strategy, and what only a human tester finds.",
    "icon": "📄",
    "section": "qa",
    "sectionKey": "qa",
    "href": "qa/manual.html",
    "path": "qa/manual.html",
    "topic": "qa/manual",
    "depth": "guide"
  },
  {
    "id": "qa/performance",
    "title": "Performance &amp;amp; Security Testing — QA &amp; Testing | TechForge",
    "desc": "Load, stress and soak testing, reading percentiles, finding bottlenecks, and the security testing that belongs in QA.",
    "icon": "📄",
    "section": "qa",
    "sectionKey": "qa",
    "href": "qa/performance.html",
    "path": "qa/performance.html",
    "topic": "qa/performance",
    "depth": "guide"
  },
  {
    "id": "qa/ui-automation",
    "title": "UI Automation — QA &amp; Testing | TechForge",
    "desc": "Selenium and Playwright compared, locator strategy, the page object model, and diagnosing flaky UI tests.",
    "icon": "📄",
    "section": "qa",
    "sectionKey": "qa",
    "href": "qa/ui-automation.html",
    "path": "qa/ui-automation.html",
    "topic": "qa/ui-automation",
    "depth": "guide"
  },
  {
    "id": "roadmaps/aiml",
    "title": "AI / ML Roadmap",
    "desc": "The AI and machine learning subject map: the vocabulary and data stack, classical ML, deep learning, then NLP, vision, r",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/aiml.html",
    "path": "roadmaps/aiml.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/cpp",
    "title": "C++ Roadmap",
    "desc": "C++ from syntax to pointers, templates and the modern standard library. Every node opens a TechForge lesson.",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/cpp.html",
    "path": "roadmaps/cpp.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/databases",
    "title": "Databases &amp; SQL Roadmap",
    "desc": "From tables and SQL through schema design, indexes, transactions and injection, then the non-relational engines and what",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/databases.html",
    "path": "roadmaps/databases.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/devops",
    "title": "DevOps Roadmap",
    "desc": "Shipping and running software: version control, containers, pipelines and the cloud underneath. Every node opens a TechF",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/devops.html",
    "path": "roadmaps/devops.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/dsa",
    "title": "DSA Roadmap",
    "desc": "The DSA roadmap as an interactive roadmap — work outward from Big-O through linear structures, trees, graphs, and techni",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/dsa.html",
    "path": "roadmaps/dsa.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/index",
    "title": "Developer Roadmaps",
    "desc": "Every TechForge roadmap in one place: seven career paths from Frontend to AI/ML, and topic-by-topic roadmaps starting wi",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/index.html",
    "path": "roadmaps/index.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/java",
    "title": "Java Roadmap",
    "desc": "Java from syntax and collections through OOP, streams and concurrency. Every node opens a TechForge lesson.",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/java.html",
    "path": "roadmaps/java.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/javascript",
    "title": "JavaScript Roadmap",
    "desc": "Every JavaScript topic on TechForge in the order the track teaches it. Every node opens a TechForge lesson.",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/javascript.html",
    "path": "roadmaps/javascript.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/networking",
    "title": "Networking Roadmap",
    "desc": "How machines talk: the stack, the transport layer, and the protocols on top. Every node opens a TechForge lesson.",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/networking.html",
    "path": "roadmaps/networking.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/os",
    "title": "Operating Systems Roadmap",
    "desc": "What runs underneath your code: processes, memory, scheduling and concurrency. Every node opens a TechForge lesson.",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/os.html",
    "path": "roadmaps/os.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/python",
    "title": "Python Roadmap",
    "desc": "The Python roadmap as a roadmap: variables and types through control flow, collections, functions, OOP, async and the st",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/python.html",
    "path": "roadmaps/python.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/security",
    "title": "Security Roadmap",
    "desc": "Threat models, cryptography, and the attacks worth knowing by name. Every node opens a TechForge lesson.",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/security.html",
    "path": "roadmaps/security.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/system-design",
    "title": "System Design Roadmap",
    "desc": "How the pieces of a large system fit together, from a single request to data at scale. Every node opens a TechForge less",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/system-design.html",
    "path": "roadmaps/system-design.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/theory",
    "title": "Theory of Computation Roadmap",
    "desc": "What machines can compute, what they cannot, and how long it takes. Every node opens a TechForge lesson.",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/theory.html",
    "path": "roadmaps/theory.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roadmaps/typescript",
    "title": "TypeScript Roadmap",
    "desc": "The TypeScript type system, from annotations through to type-level programming. Every node opens a TechForge lesson.",
    "icon": "📄",
    "section": "roadmaps",
    "sectionKey": "roadmaps",
    "href": "roadmaps/typescript.html",
    "path": "roadmaps/typescript.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "roles/index",
    "title": "Learn by Role",
    "desc": "Pick a career and follow it end to end: frontend, backend, full stack, mobile, games, AI and ML, data science, data engi",
    "icon": "📄",
    "section": "roles",
    "sectionKey": "roles",
    "href": "roles/index.html",
    "path": "roles/index.html",
    "topic": null,
    "depth": "page"
  },
  {
    "id": "systems/app-security",
    "title": "Secure Development — Security | TechForge",
    "desc": "Secure Development — Security | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "security/app-security.html",
    "path": "systems/security/app-security.html",
    "topic": "sys/sec-appsec",
    "depth": "guide"
  },
  {
    "id": "systems/authentication",
    "title": "Authentication &amp; Authorization — Security | TechForge",
    "desc": "Authentication &amp; Authorization — Security | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "security/authentication.html",
    "path": "systems/security/authentication.html",
    "topic": "sys/sec-auth",
    "depth": "guide"
  },
  {
    "id": "systems/crypto",
    "title": "Cryptography Basics — Security | TechForge",
    "desc": "Cryptography Basics — Security | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "security/crypto.html",
    "path": "systems/security/crypto.html",
    "topic": "sys/sec-crypto",
    "depth": "guide"
  },
  {
    "id": "systems/fundamentals",
    "title": "Security Fundamentals — Security | TechForge",
    "desc": "Security Fundamentals — Security | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "security/fundamentals.html",
    "path": "systems/security/fundamentals.html",
    "topic": "sys/sec-fundamentals",
    "depth": "guide"
  },
  {
    "id": "systems/incident-response",
    "title": "Attacks &amp; Incident Response — Security | TechForge",
    "desc": "Attacks &amp; Incident Response — Security | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "security/incident-response.html",
    "path": "systems/security/incident-response.html",
    "topic": "sys/sec-incident",
    "depth": "guide"
  },
  {
    "id": "systems/index",
    "title": "Security",
    "desc": "Security roadmap — fundamentals and threat modeling, cryptography, authentication, OWASP web attacks, TLS/PKI, network s",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "security/index.html",
    "path": "systems/security/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "systems/network-security",
    "title": "Network Security — Security | TechForge",
    "desc": "Network Security — Security | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "security/network-security.html",
    "path": "systems/security/network-security.html",
    "topic": "sys/sec-network",
    "depth": "guide"
  },
  {
    "id": "systems/tls-pki",
    "title": "TLS &amp; PKI — Security | TechForge",
    "desc": "TLS &amp; PKI — Security | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "security/tls-pki.html",
    "path": "systems/security/tls-pki.html",
    "topic": "sys/sec-tls",
    "depth": "guide"
  },
  {
    "id": "systems/web-security",
    "title": "Web Attacks — OWASP Essentials — Security | TechForge",
    "desc": "Web Attacks — OWASP Essentials — Security | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "security/web-security.html",
    "path": "systems/security/web-security.html",
    "topic": "sys/sec-web",
    "depth": "guide"
  },
  {
    "id": "system-design/api-gateway",
    "title": "API Gateway — System Design",
    "desc": "API Gateway — System Design guide on TechForge. Theory, examples, interview questions, and best practices.",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/api-gateway.html",
    "path": "system-design/api-gateway.html",
    "topic": "sd/api-gateway",
    "depth": "guide"
  },
  {
    "id": "system-design/caching",
    "title": "Caching Strategies — System Design",
    "desc": "Caching stores frequently accessed data in a fast layer (memory, CDN edge) to reduce latency and backend load. The funda",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/caching.html",
    "path": "system-design/caching.html",
    "topic": "sd/caching",
    "depth": "guide"
  },
  {
    "id": "system-design/cap-theorem",
    "title": "CAP Theorem — System Design",
    "desc": "The CAP theorem states that during a network partition, a distributed system cannot simultaneously guarantee both Consis",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/cap-theorem.html",
    "path": "system-design/cap-theorem.html",
    "topic": "sd/cap-theorem",
    "depth": "guide"
  },
  {
    "id": "system-design/cdn",
    "title": "CDN — System Design",
    "desc": "A Content Delivery Network caches content at edge PoPs close to users — cutting latency and shielding origin servers fro",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/cdn.html",
    "path": "system-design/cdn.html",
    "topic": "sd/cdn",
    "depth": "guide"
  },
  {
    "id": "system-design/consistency-models",
    "title": "Consistency Models — System Design",
    "desc": "Strong consistency guarantees that after a write completes, all subsequent reads return the latest value — the system be",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/consistency-models.html",
    "path": "system-design/consistency-models.html",
    "topic": "sd/consistency-models",
    "depth": "guide"
  },
  {
    "id": "system-design/database-sharding",
    "title": "Database Sharding — System Design",
    "desc": "Database sharding horizontally partitions rows across multiple database instances when a single node cannot meet storage",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/database-sharding.html",
    "path": "system-design/database-sharding.html",
    "topic": "sd/database-sharding",
    "depth": "guide"
  },
  {
    "id": "system-design/distributed-systems",
    "title": "Distributed Systems — System Design",
    "desc": "Distributed systems coordinate multiple independent nodes that present a unified service. Partial failures are normal — ",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/distributed-systems.html",
    "path": "system-design/distributed-systems.html",
    "topic": "sd/distributed-systems",
    "depth": "guide"
  },
  {
    "id": "system-design/django",
    "title": "Django Framework",
    "desc": "Django framework tutorial — MVT architecture, ORM, admin panel, authentication, and deployment.",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/django.html",
    "path": "system-design/django.html",
    "topic": "sd/django",
    "depth": "guide"
  },
  {
    "id": "system-design/event-driven",
    "title": "Event-Driven Systems — System Design",
    "desc": "Event-driven architecture uses immutable events as the integration contract between services. Producers emit facts (&quo",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/event-driven.html",
    "path": "system-design/event-driven.html",
    "topic": "sd/event-driven",
    "depth": "guide"
  },
  {
    "id": "system-design/fastapi",
    "title": "FastAPI Framework",
    "desc": "FastAPI tutorial — async Python API framework with automatic Swagger docs, Pydantic validation, dependency injection, JW",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/fastapi.html",
    "path": "system-design/fastapi.html",
    "topic": "sd/fastapi",
    "depth": "guide"
  },
  {
    "id": "system-design/fault-tolerance",
    "title": "Fault Tolerance — System Design",
    "desc": "Fault tolerance means the system continues to deliver core functionality when components fail — through redundancy, isol",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/fault-tolerance.html",
    "path": "system-design/fault-tolerance.html",
    "topic": "sd/fault-tolerance",
    "depth": "guide"
  },
  {
    "id": "system-design/flask",
    "title": "Flask Framework",
    "desc": "Flask microframework tutorial — routes, Jinja2 templates, forms, Flask-SQLAlchemy, Blueprints, REST APIs, and deployment",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/flask.html",
    "path": "system-design/flask.html",
    "topic": "sd/flask",
    "depth": "guide"
  },
  {
    "id": "system-design/fundamentals",
    "title": "System Design Fundamentals — System Design",
    "desc": "System design is the process of defining a system's architecture to meet functional requirements (what it does) and non-",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/fundamentals.html",
    "path": "system-design/fundamentals.html",
    "topic": "sd/fundamentals",
    "depth": "guide"
  },
  {
    "id": "system-design/high-availability",
    "title": "High Availability — System Design",
    "desc": "High availability minimizes downtime through redundancy and automatic failover. Expressed as &quot;nines&quot;: 99.9% al",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/high-availability.html",
    "path": "system-design/high-availability.html",
    "topic": "sd/high-availability",
    "depth": "guide"
  },
  {
    "id": "system-design/index",
    "title": "System Design",
    "desc": "System Design — scalable architecture, caching, load balancing, microservices, distributed systems, plus Django/Flask/Fa",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/index.html",
    "path": "system-design/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "system-design/kafka",
    "title": "Apache Kafka — System Design",
    "desc": "Apache Kafka is a distributed commit log for event streaming. Producers append records to topic partitions; consumers re",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/kafka.html",
    "path": "system-design/kafka.html",
    "topic": "sd/kafka",
    "depth": "guide"
  },
  {
    "id": "system-design/load-balancing",
    "title": "Load Balancing — System Design",
    "desc": "Load balancers distribute incoming traffic across multiple backend servers to improve availability, throughput, and faul",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/load-balancing.html",
    "path": "system-design/load-balancing.html",
    "topic": "sd/load-balancing",
    "depth": "guide"
  },
  {
    "id": "system-design/microservices",
    "title": "Microservices — System Design",
    "desc": "Microservices decompose an application into independently deployable services bounded by business domains (DDD). Each se",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/microservices.html",
    "path": "system-design/microservices.html",
    "topic": "sd/microservices",
    "depth": "guide"
  },
  {
    "id": "system-design/monolith",
    "title": "Monolith — System Design",
    "desc": "A monolith deploys all application logic as a single unit — shared process, shared database, unified release cycle. It i",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/monolith.html",
    "path": "system-design/monolith.html",
    "topic": "sd/monolith",
    "depth": "guide"
  },
  {
    "id": "system-design/partitioning",
    "title": "Partitioning — System Design",
    "desc": "Partitioning splits data across nodes by a partition key so no single machine holds the entire dataset. It is the primar",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/partitioning.html",
    "path": "system-design/partitioning.html",
    "topic": "sd/partitioning",
    "depth": "guide"
  },
  {
    "id": "system-design/queues",
    "title": "Message Queues — System Design",
    "desc": "Message queues decouple producers from consumers by storing messages until a consumer is ready to process them. This ena",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/queues.html",
    "path": "system-design/queues.html",
    "topic": "sd/queues",
    "depth": "guide"
  },
  {
    "id": "system-design/rabbitmq",
    "title": "RabbitMQ — System Design",
    "desc": "RabbitMQ is an AMQP message broker routing messages from producers to consumers via exchanges and queues. Unlike Kafka's",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/rabbitmq.html",
    "path": "system-design/rabbitmq.html",
    "topic": "sd/rabbitmq",
    "depth": "guide"
  },
  {
    "id": "system-design/rate-limiting",
    "title": "Rate Limiting — System Design",
    "desc": "Rate limiting caps how many requests a client can make per time window — protecting backends from overload, abuse, and c",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/rate-limiting.html",
    "path": "system-design/rate-limiting.html",
    "topic": "sd/rate-limiting",
    "depth": "guide"
  },
  {
    "id": "system-design/replication",
    "title": "Replication — System Design",
    "desc": "Replication copies data to multiple nodes for fault tolerance and read scaling. Leader-follower (primary-replica) is the",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/replication.html",
    "path": "system-design/replication.html",
    "topic": "sd/replication",
    "depth": "guide"
  },
  {
    "id": "system-design/reverse-proxy",
    "title": "Reverse Proxy — System Design",
    "desc": "A reverse proxy sits between clients and backend servers, accepting requests on behalf of the backends. Unlike a forward",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/reverse-proxy.html",
    "path": "system-design/reverse-proxy.html",
    "topic": "sd/reverse-proxy",
    "depth": "guide"
  },
  {
    "id": "system-design/scalability",
    "title": "Scalability — System Design",
    "desc": "Scalability is the ability to handle increasing load by adding resources. Vertical scaling (bigger CPU/RAM/disk) is quic",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/scalability.html",
    "path": "system-design/scalability.html",
    "topic": "sd/scalability",
    "depth": "guide"
  },
  {
    "id": "system-design/service-discovery",
    "title": "Service Discovery — System Design",
    "desc": "Service discovery lets clients locate healthy instances of a service without hardcoded IPs — critical when containers an",
    "icon": "🏗️",
    "section": "System Design",
    "sectionKey": "system-design",
    "href": "system-design/service-discovery.html",
    "path": "system-design/service-discovery.html",
    "topic": "sd/service-discovery",
    "depth": "guide"
  },
  {
    "id": "systems/index",
    "title": "OS &amp; Networks",
    "desc": "OS &amp; Networks hub — operating systems (processes, memory, scheduling), networking (TCP/IP, HTTP, DNS), security (cry",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "systems/index.html",
    "path": "systems/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "systems/automata",
    "title": "Finite Automata — Theory of Computation | TechForge",
    "desc": "Finite Automata — Theory of Computation | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "theory/automata.html",
    "path": "systems/theory/automata.html",
    "topic": "sys/toc-automata",
    "depth": "guide"
  },
  {
    "id": "systems/complexity",
    "title": "Time Complexity — Theory of Computation | TechForge",
    "desc": "Time Complexity — Theory of Computation | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "theory/complexity.html",
    "path": "systems/theory/complexity.html",
    "topic": "sys/toc-complexity",
    "depth": "guide"
  },
  {
    "id": "systems/decidability",
    "title": "Decidability &amp; the Halting Problem — Theory of Computation | TechForge",
    "desc": "Decidability &amp; the Halting Problem — Theory of Computation | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "theory/decidability.html",
    "path": "systems/theory/decidability.html",
    "topic": "sys/toc-decidability",
    "depth": "guide"
  },
  {
    "id": "systems/grammars",
    "title": "Grammars &amp; Parsing — Theory of Computation | TechForge",
    "desc": "Grammars &amp; Parsing — Theory of Computation | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "theory/grammars.html",
    "path": "systems/theory/grammars.html",
    "topic": "sys/toc-grammars",
    "depth": "guide"
  },
  {
    "id": "systems/index",
    "title": "Theory of Computation",
    "desc": "Theory of computation roadmap — finite automata, regular languages and regex, grammars and parsing, Turing machines, dec",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "theory/index.html",
    "path": "systems/theory/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "systems/p-vs-np",
    "title": "P vs NP — Theory of Computation | TechForge",
    "desc": "P vs NP — Theory of Computation | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "theory/p-vs-np.html",
    "path": "systems/theory/p-vs-np.html",
    "topic": "sys/toc-pnp",
    "depth": "guide"
  },
  {
    "id": "systems/reductions",
    "title": "Hardness in the Wild — Theory of Computation | TechForge",
    "desc": "Hardness in the Wild — Theory of Computation | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "theory/reductions.html",
    "path": "systems/theory/reductions.html",
    "topic": "sys/toc-reductions",
    "depth": "guide"
  },
  {
    "id": "systems/regular-languages",
    "title": "Regular Languages &amp; Regex — Theory of Computation | TechForge",
    "desc": "Regular Languages &amp; Regex — Theory of Computation | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "theory/regular-languages.html",
    "path": "systems/theory/regular-languages.html",
    "topic": "sys/toc-regular",
    "depth": "guide"
  },
  {
    "id": "systems/turing-machines",
    "title": "Turing Machines — Theory of Computation | TechForge",
    "desc": "Turing Machines — Theory of Computation | TechForge",
    "icon": "📄",
    "section": "systems",
    "sectionKey": "systems",
    "href": "theory/turing-machines.html",
    "path": "systems/theory/turing-machines.html",
    "topic": "sys/toc-turing",
    "depth": "guide"
  },
  {
    "id": "programming/advanced-types",
    "title": "TypeScript Advanced Types — Utility, Mapped &amp; Conditional | TechForge",
    "desc": "TypeScript Advanced Types — Utility, Mapped &amp; Conditional | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/advanced-types.html",
    "path": "programming/typescript/advanced-types.html",
    "topic": "ts/advanced-types",
    "depth": "guide"
  },
  {
    "id": "programming/basics",
    "title": "TypeScript Basics — Types &amp; Inference | TechForge",
    "desc": "TypeScript Basics — Types &amp; Inference | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/basics.html",
    "path": "programming/typescript/basics.html",
    "topic": "ts/basics",
    "depth": "guide"
  },
  {
    "id": "programming/functions",
    "title": "TypeScript Functions &amp; Narrowing | TechForge",
    "desc": "TypeScript Functions &amp; Narrowing | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/functions.html",
    "path": "programming/typescript/functions.html",
    "topic": "ts/functions",
    "depth": "guide"
  },
  {
    "id": "programming/generics",
    "title": "TypeScript Generics | TechForge",
    "desc": "TypeScript Generics | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/generics.html",
    "path": "programming/typescript/generics.html",
    "topic": "ts/generics",
    "depth": "guide"
  },
  {
    "id": "programming/index",
    "title": "TypeScript Guide",
    "desc": "Complete TypeScript guide from types to advanced type-level programming — inference, generics, discriminated unions, uti",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/index.html",
    "path": "programming/typescript/index.html",
    "topic": null,
    "depth": "guide"
  },
  {
    "id": "programming/objects",
    "title": "TypeScript Objects &amp; Interfaces | TechForge",
    "desc": "TypeScript Objects &amp; Interfaces | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/objects.html",
    "path": "programming/typescript/objects.html",
    "topic": "ts/objects",
    "depth": "guide"
  },
  {
    "id": "programming/oop",
    "title": "TypeScript Classes &amp; OOP | TechForge",
    "desc": "TypeScript Classes &amp; OOP | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/oop.html",
    "path": "programming/typescript/oop.html",
    "topic": "ts/oop",
    "depth": "guide"
  },
  {
    "id": "programming/programs",
    "title": "TypeScript Programs — Practice Code Examples | TechForge",
    "desc": "TypeScript Programs — Practice Code Examples | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/programs.html",
    "path": "programming/typescript/programs.html",
    "topic": "ts/programs",
    "depth": "guide"
  },
  {
    "id": "programming/tooling",
    "title": "TypeScript Tooling — tsconfig, strict &amp; Declaration Files | TechForge",
    "desc": "TypeScript Tooling — tsconfig, strict &amp; Declaration Files | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/tooling.html",
    "path": "programming/typescript/tooling.html",
    "topic": "ts/tooling",
    "depth": "guide"
  },
  {
    "id": "programming/unions",
    "title": "TypeScript Unions, Enums &amp; Discriminated Unions | TechForge",
    "desc": "TypeScript Unions, Enums &amp; Discriminated Unions | TechForge",
    "icon": "📄",
    "section": "programming",
    "sectionKey": "programming",
    "href": "typescript/unions.html",
    "path": "programming/typescript/unions.html",
    "topic": "ts/unions",
    "depth": "guide"
  }
];

  if (window.TF_TOPIC_INDEX && window.TF_SITE_INDEX) {
    var seen = {};
    window.TF_TOPIC_INDEX.forEach(function (t) { seen[t.href] = true; });
    window.TF_SITE_INDEX.forEach(function (p) {
      if (!seen[p.href]) window.TF_TOPIC_INDEX.push(p);
    });
  } else if (window.TF_SITE_INDEX && !window.TF_TOPIC_INDEX) {
    window.TF_TOPIC_INDEX = window.TF_SITE_INDEX.slice();
  }
})();
